...
async function getBasicInfo(doc) {
	...
	const basicInfo = mainRows.reduce((acc, row) => {
		...
		try {
			...
			console.error(
				`Error parsing basic info for member "${firstName} ${lastName}" (${id}): ${e.message}`,
			);
			process.exit(1);
			return acc;
		}
...
function parseSeminar({ event, date, note, instructor }) {
	return {
		name: note,
		date,
...
function parseAndMergeTaikai(
	acc,
	{ event, date, taikaiLocation, taikaiYear, win1, win2, win3, win4 },
) {
...
async function getSeminarInfo(doc) {
	...
	const info = rows.reduce((acc, row) => {
		...
		try {
			...
			console.error(
				`Error parsing seminar data for member ${id}: ${e.message}`,
			);
			process.exit(1);
			return acc;
		}
